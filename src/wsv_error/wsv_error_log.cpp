...
static void
ignore_retval_write(int fd, const void *buf, size_t count)
{
...
{
	wsv_error_record(m);
	ignore_retval_write(STDERR_FILENO, (void*)g_error_buf, g_error_len);
	wsv_exit(WSV_EXIT_ERR_LOG);
}
...
{
	wsv_error_record(m);
	if (write(g_error_log_fd, (void*)g_error_buf, g_error_len) == -1)
...
{
	if (g_error_log_fd >= 0 && close(g_error_log_fd) == -1)
